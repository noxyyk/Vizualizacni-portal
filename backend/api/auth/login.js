...
router.post('/', async (req, res) => {
    ...
    if (!(await auth.checkIfExists(req.body.username))) return res.status(409).send({valid: false, response: "Uživatel s tímto jménem nexistuje"});
    if (auth.authenticateUser(req.body.username, req.body.password)) return res.status(401).send({valid: false, response: "Heslo se neshhoduje"});
